...
async fn main() -> anyhow::Result<()> {
    ...
    */
    ...
    loop {
        ...
        // NOTE: Remember that the compiler transforms an async block into a Future state machine. That's why we can pass it to spawn().
        /*
        We use tokio::task::spawn(...) in order not to block the loop. This way, we can accept more than one
        connection at a time.
        */
        tokio::task::spawn(async {
            handle_connection(socket).await?;
            // This line below is just because we want to be able to use `?` in the above line.
            Ok::<_, anyhow::Error>(())
        });
    }
}
